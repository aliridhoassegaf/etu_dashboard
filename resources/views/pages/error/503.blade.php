<!doctype html>
{{-- error/503 — faithful re-expression of src/html/error/503.html. Standalone
     status page (no shell); same DOM/classes/ARIA. --}}
<html lang="en" data-ax-route="error/503" data-error-code="503">
...
<body class="ax-standalone" data-ax-layout="status">
  ...
  <main class="ax-center" id="ax-main" style="position:relative;z-index:1;width:100%;padding:var(--ax-space-6);">
    <div style="width:100%;max-width:520px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:var(--ax-space-6);">
      ...
      <!-- thin-line illustration: tools / paused service (highlight tinted warning per single-highlight rule) -->
      <div aria-hidden="true" style="position:relative;display:grid;place-items:center;width:160px;height:160px;border-radius:50%;background:radial-gradient(circle at 50% 40%, color-mix(in oklab, var(--ax-warning-500) 14%, transparent), transparent 70%);">
        <span style="position:absolute;inset:0;border-radius:50%;border:1px dashed var(--ax-border-strong);"></span>
        <svg viewBox="0 0 24 24" width="68" height="68" fill="none" stroke="var(--ax-text-muted)" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
        ...
        <svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="var(--ax-warning-500)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="position:absolute;right:34px;top:38px;">
      ...
      <div>
        <h1 style="margin:0;display:flex;flex-direction:column;align-items:center;gap:var(--ax-space-2);">
          <span class="ax-num" style="font-family:var(--ax-font-mono);font-weight:600;font-size:var(--ax-text-4xl);line-height:1;color:var(--ax-text-strong);letter-spacing:.04em;">503</span>
          <span style="font-family:var(--ax-font-display);font-weight:600;font-size:var(--ax-text-2xl);color:var(--ax-text-strong);">Service unavailable</span>
        </h1>
        <p style="margin:var(--ax-space-4) auto 0;max-width:44ch;font-size:var(--ax-text-md);color:var(--ax-text-muted);line-height:1.55;">
          The service is temporarily unavailable — it may be under maintenance. Everything should be back shortly. Thanks for your patience.
        </p>
      </div>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:var(--ax-space-3);justify-content:center;">
        <button type="button" class="ax-btn ax-btn--primary" onclick="window.location.reload()">
          ...
          <span class="ax-btn__label">Try again</span>
        </button>
        <a class="ax-btn ax-btn--secondary" href="/auth/maintenance">
          <svg class="ax-btn__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 12a9 9 0 1 0 18 0a9 9 0 1 0 -18 0"/><path d="M12 8v4l3 3"/></svg>
          <span class="ax-btn__label">View maintenance status</span>
        </a>
      </div>

      <!-- error reference id (copy-on-click) + expected window -->
      <div x-data="{ ref:'SVC-503-2C7B14', copied:false, copy(){ navigator.clipboard?.writeText(this.ref).then(()=>{this.copied=true; setTimeout(()=>this.copied=false,1600)}) } }"
           style="display:flex;flex-direction:column;align-items:center;gap:var(--ax-space-3);">
        <p style="margin:0;font-size:var(--ax-text-sm);color:var(--ax-text-subtle);">Estimated back online by <b class="ax-num" style="font-family:var(--ax-font-mono);color:var(--ax-text);">14:30 UTC</b></p>
        <div style="display:flex;align-items:center;gap:var(--ax-space-2);font-size:var(--ax-text-sm);color:var(--ax-text-subtle);">
          ...
          <button type="button" @click="copy()"
                  ...
                  :aria-label="copied ? 'Reference ID copied' : 'Copy reference ID SVC-503-2C7B14'">
            <span class="ax-num" x-text="ref">SVC-503-2C7B14</span>
